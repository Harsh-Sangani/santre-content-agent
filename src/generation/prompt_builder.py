...
def build_caption_prompt(theme: DayTheme, brand_positioning: str) -> str:
    ...
    return (
        f"Look at the provided image first. Write the message text that will "
        f"accompany this exact image when it's posted to a WhatsApp "
        f"community/group for a jewellery brand. This is NOT an Instagram "
        f"caption -- there are no comments, no hashtags, no 'link in bio', "
        f"no follow/like mechanics, and no swipe-up. Write it as a natural, "
        f"direct message someone would actually send in a WhatsApp broadcast. "
        f"Only describe what is actually depicted in the image -- if it shows "
        f"earrings, call them earrings; if it shows a bracelet, call it a "
        f"bracelet; do not name or describe a different piece type (e.g. a "
        f"pendant or ring) unless that is what's actually shown. "
        f"Theme: {theme.theme_name}. Content focus: {theme.content_focus} "
        f"Brand voice and positioning to follow: {brand_positioning} "
        f"{HUMAN_TONE_INSTRUCTION} "
